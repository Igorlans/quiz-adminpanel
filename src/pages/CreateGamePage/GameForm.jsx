...
import { addGame, removeGame, updateGame } from "./parseGame.js";
...
const GameForm = ({ game }) => {
    ...
    const updateValues = {
        ...
        toStarsIndex: game?.toStarsIndex,
        finalScreen: game?.finalScreen,
        tours: game?.tours
    }
    ...
    const submitHandler = async (data) => {
        if (!game?.image && !file) return toast.error('Додайте фото')
        ...
        await toast.promise(isUpdate ? updateGame(data, file || game?.image, game?.roundIds, game?.id) : addGame(data, file), {
            loading: isUpdate ? 'Оновлення гри...' : 'Створення гри...',
            success: () => {
                navigate('/games')
                return isUpdate ? 'Гра оновлена' : 'Гра створена'
            },
            error: (e) => e.message
        })
    ...
    const removeHandler = async () => {
        await toast.promise(removeGame(game?.id, game?.roundIds), {
            loading: 'Видалення гри...',
            success: () => {
                navigate('/games')
                return 'Гра видалена'
            },
    ...
    return (
        <>
            ...
            <PageContainer>
                <FormBody>
                    <FormHeader>
                        <PageTitle>Оновлення гри</PageTitle>
                    ...
                    <Form>
                        ...
                        <FormUploader>
                            ...
                            <Cropper placeholder={game?.image || cropperPlaceholder} file={file} setFile={setFile} aspect={1} height={200} width={200} />
                        </FormUploader>
                    </Form>
